...
class WeatherPage extends Component {
  constructor(props) {
    ...
    this.Place = [{name: '札幌', id: 2128295}, {name: '東京', id:1850147}, {name: '大阪', id: 1853909}, {name: '沖縄', id:1894616}]
    this.OpenWeatherMapKey = 'APIKEY'
  }

  selectPlace(index) {
    if (index > 0) {
      const place = this.Place[index - 1]
    ...
    return (
      <MuiThemeProvider>
        <Card style={{margin: 30}}>
          ...
          <CardActions>
            <PlaceSelector places={this.Place} actionSelect={(ix) => this.selectPlace(ix)} />
          </CardActions>
        </Card>
      </MuiThemeProvider>
    )
...
const Title = (props) => {
  return (<h1>{props.place ? props.place + 'の天気' : '天気情報'}</h1>)
}
Title.propTypes = {
